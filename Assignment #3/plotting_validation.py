# -*- coding: utf-8 -*-
"""Plotting validation.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1PwYNAa5QbYd9Vm1BQdHQlCqCUHMP3mRp
"""
...
each_task_val = np.asarray([val.images[:,task_permutation[i]] for i in range(num_tasks_to_run)])
...
def plot_weights(epoch_each_task_validation):
    ...
    for i in range(task+1):
      ...
      plt.title(f'Train for task{i}')
      # Set the label for the sub-plot.
      plt.xlabel('epoch')
      plt.ylabel(f"accuracy")
      # Plot the image.
      plt.plot([i+1 for i in range(epoch_each_task_validation.shape[0])], epoch_each_task_validation[:,i],
...
for task in range(num_tasks_to_run):
  ...
  if task == 0:
    ...
    n_epochs = 20
  epoch_each_task_validation = list()
  ...
  #     print(mlp_on_cpu.loss(preds, label))
    ...
    each_task_validation = list()
    for each in range(task+1):
      val_data = each_task_val[each]
      preds = mlp_on_cpu.forward(val_data)
      correct_preds = tf.equal(tf.argmax(tf.nn.softmax(preds), 1), tf.argmax(val_label, 1))
      accuracy = tf.reduce_mean(tf.cast(correct_preds, tf.float32))
      each_task_validation.append(accuracy)
    epoch_each_task_validation.append(each_task_validation)
  epoch_each_task_validation = np.asarray(epoch_each_task_validation)
  print(epoch_each_task_validation)
  plot_weights(epoch_each_task_validation)
  row_of_R=list()
